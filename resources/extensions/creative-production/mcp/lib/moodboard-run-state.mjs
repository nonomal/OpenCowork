...
const PREVIEW_SCRIPT_CANDIDATES = [
  process.env.CREATIVE_PRODUCTION_PREVIEW_IMAGE_SCRIPT,
  pluginPath("mcp", "scripts", "create_preview_image.py"),
  path.resolve(MODULE_DIR, "..", "scripts", "create_preview_image.py"),
].filter(Boolean);
...
async function normalizeAppendedItemImage(runDirectory, item = {}) {
  ...
  if (!id) {
    return {
      ...item,
      id,
      imageUrl: appendDisplaySource(item),
    };
  }

  return materializeMoodboardImage(runDirectory, { ...item, id });
}

function appendDisplaySource(item = {}) {
  ...
  if (pathValue && path.isAbsolute(pathValue)) return pathValue;
  return imageUrl || urlValue || pathValue;
}

function appendLocalImageSource(item = {}) {
  const sourceImageUrl = String(item.sourceImageUrl || "").trim();
  const pathValue = String(item.path || "").trim();
  const imageUrl = String(item.imageUrl || "").trim();
  const urlValue = String(item.url || "").trim();

  if (sourceImageUrl && (isRunLocalGeneratedImage(sourceImageUrl) || path.isAbsolute(sourceImageUrl))) {
    return sourceImageUrl;
  }
  if (pathValue && (isRunLocalGeneratedImage(pathValue) || path.isAbsolute(pathValue))) {
    return pathValue;
  }
  if (imageUrl && (isRunLocalGeneratedImage(imageUrl) || path.isAbsolute(imageUrl))) {
    return imageUrl;
  }
  if (urlValue && (isRunLocalGeneratedImage(urlValue) || path.isAbsolute(urlValue))) {
    return urlValue;
  }
  return "";
}

async function materializeMoodboardImage(runDirectory, item = {}) {
  const id = String(item.id || "").trim();
  const localSource = appendLocalImageSource(item);
  if (!localSource) {
    return {
      ...item,
      id,
  ...
  const runImageUrl = await copyImageIntoGenerated(runDirectory, id, localSource);
  const normalized = await normalizeRunLocalGeneratedItem(runDirectory, {
    ...item,
    id,
  }, runImageUrl);
  delete normalized.path;
  if (String(item.url || "").trim() && path.isAbsolute(String(item.url).trim())) {
    delete normalized.url;
  }
  return normalized;
}

async function copyImageIntoGenerated(runDirectory, id, localSource) {
  if (isRunLocalGeneratedImage(localSource)) {
    const normalizedSource = localSource.startsWith("/") ? localSource : `/${localSource}`;
    resolveRunPath(runDirectory, normalizedSource.replace(/^\/+/, ""));
    return normalizedSource;
  }

  const resolvedLocalImagePath = resolveRunPath(runDirectory, localSource);
  ...
  return `/generated/${path.basename(generatedPath)}`;
}
...
async function normalizeRunLocalGeneratedItem(runDirectory, item, sourceImageUrl) {
  const normalizedSourceImageUrl = sourceImageUrl.startsWith("/")
    ? sourceImageUrl
    : `/${sourceImageUrl}`;
  const previewResult = await previewImageForWidget(runDirectory, item, normalizedSourceImageUrl, { requirePreview: true });
  if (previewResult.ok) {
    return {
      ...item,
  ...
  throw new Error(
    [
      "Mood-board image import failed before rendering.",
      previewResult.error || "Image preview could not be created for the MCP mood-board widget.",
      "Expected import contract: copy the source image under the runDirectory, create a JPEG preview under generated/mcp-thumbs/, set imageUrl to /generated/mcp-thumbs/<id>.jpg, and keep the full-resolution source in sourceImageUrl.",
    ].join(" "),
  );
}
...
async function syncRunStateAfterAppend({
  ...
  const hiddenImageIds = hiddenImageIdsFromRunState(existingState, itemIds);
  const hiddenIdSet = new Set(hiddenImageIds);
  const selectedImageIds = Array.isArray(existingState.selectedImageIds)
    ? existingState.selectedImageIds.map(String).filter((id) => itemIdSet.has(id) && !hiddenIdSet.has(id))
    : [];
  const visibleItemIds = itemIds.filter((id) => !hiddenIdSet.has(id));
  const existingCurrentImageId = String(existingState.currentImageId || "");
  const currentImageId = itemIdSet.has(existingCurrentImageId) && !hiddenIdSet.has(existingCurrentImageId)
    ? String(existingState.currentImageId)
    : lastVisibleId(appendedItemIds, itemIdSet, hiddenIdSet) || lastVisibleId(itemIds, itemIdSet, hiddenIdSet) || "";
  const timestamp = new Date().toISOString();
  const runState = {
    ...existingState,
...
function hiddenImageIdsFromRunState(existingState, itemIds) {
  const itemIdSet = new Set(itemIds);
  const hiddenIds = new Set(
    Array.isArray(existingState.hiddenImageIds)
      ? existingState.hiddenImageIds.map(String).filter((id) => itemIdSet.has(id))
      : [],
  );

  const visibleIds = Array.isArray(existingState.visibleItemIds)
    ? new Set(existingState.visibleItemIds.map(String).filter((id) => itemIdSet.has(id)))
    : null;
  const previousItemIds = Array.isArray(existingState.items)
    ? existingState.items.map((item) => String(item?.id || "")).filter((id) => itemIdSet.has(id))
    : [];

  if (visibleIds && previousItemIds.length > 0 && visibleIds.size < previousItemIds.length) {
    previousItemIds.forEach((id) => {
      if (!visibleIds.has(id)) hiddenIds.add(id);
    });
  }

  return itemIds.filter((id) => hiddenIds.has(id));
}

function lastVisibleId(ids, itemIdSet, hiddenIdSet) {
  for (let index = ids.length - 1; index >= 0; index -= 1) {
    const id = String(ids[index] || "");
    if (itemIdSet.has(id) && !hiddenIdSet.has(id)) return id;
  }
  return "";
}
...
async function previewImageUrlForWidget(runDirectory, item, imageUrl) {
  const previewResult = await previewImageForWidget(runDirectory, item, imageUrl);
  return previewResult.ok ? previewResult.imageUrl : imageUrl;
}

async function previewImageForWidget(runDirectory, item, imageUrl, { requirePreview = false } = {}) {
  if (!isRunLocalGeneratedImage(imageUrl) || imageUrl.includes(`/${PREVIEW_DIR}/`)) {
    return { ok: true, imageUrl };
  }
  const sourcePath = resolveRunPath(runDirectory, imageUrl.replace(/^\/+/, ""));
  const extension = path.extname(sourcePath).toLowerCase();
  if (!imageMimeTypes.has(extension) || extension === ".svg") {
    if (requirePreview) {
      return {
        ok: false,
        imageUrl,
        error: `Image preview could not be created: unsupported image type ${extension || "(none)"}`,
      };
    }
    return { ok: true, imageUrl };
  }
  ...
  try {
    ...
    return { ok: true, imageUrl: `/generated/${PREVIEW_DIR}/${path.basename(previewPath)}` };
  } catch (error) {
    return {
      ok: false,
      imageUrl,
      error: `Image preview could not be created: ${error.message}`,
    };
  }
}
...
async function createPreviewImage(sourcePath, previewPath, { maxEdge, quality }) {
  const previewScriptPath = await resolvePreviewScriptPath();
  try {
    await execFileAsync(
    ...
    throw new Error(
      [
        ...
        "Install plugin Python dependencies from requirements.txt before appending local images to a mood board.",
        `Python command: ${process.env.CREATIVE_PRODUCTION_PYTHON || "python3"}.`,
...
async function resolvePreviewScriptPath() {
  for (const candidate of PREVIEW_SCRIPT_CANDIDATES) {
    try {
  ...
  throw new Error(`Preview helper script was not found. Checked: ${PREVIEW_SCRIPT_CANDIDATES.join(", ")}`);
}
