...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create ~/.open-cowork/state/plugins/product-design/user-context.md "
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="OpenCowork home directory. Defaults to ~/.open-cowork or ~/.open-cowork.",
    )
    parser.add_argument(
        "--state-dir",
        type=Path,
...
def resolve_state_dir(home_dir: Path | None, state_dir: Path | None) -> Path:
    if state_dir is not None:
        return state_dir.expanduser().resolve()
    home = home_dir or Path(os.environ.get("OPEN_COWORK_HOME", "~/.open-cowork"))
