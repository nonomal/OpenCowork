...
Follow the shared Product Design routing guidance in [$index](../product-design-index/SKILL.md).

## Critical Overrides

- Refer to the Plugin router [$index](../product-design-index/SKILL.md) before proceeding.
- Follow [$critical-overrides](references/critical-overrides.md).

## User Context

Before starting, load [$user-context](../product-design-user-context/SKILL.md) and run its preflight script when local shell access is available.
...
Do not generate images until `$get-context` has played back and confirmed the design brief for this exact request. If this skill was invoked directly and the current thread does not already contain that confirmed brief, route to [$get-context](../product-design-get-context/SKILL.md) first.
